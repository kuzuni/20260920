...
- Terrain is one full rectangular opaque sprite across the arena and camera margins. DoodleTerrain samples the selected Grounds atlas region through mirrored world coordinates with a 3-unit pattern. _MainTex holds sprite geometry compatibility; _AtlasTex holds the actual theme atlas. This avoids tight-mesh tile gaps and reduces motifs from the previous 13-unit size. Each theme now has an explicit muted base palette; atlas detail uses 0.16 strength and 0.4 saturation so its black outlines and bright marks stay behind characters. The output remains fully opaque, including transparent source pixels.
