    ...
    public partial class DoodleIdlePlayModeTests
    {
        ...
        public IEnumerator ExpansionCatalogsDetailScaleAndSummonRowsKeepTheirLayout()
        {
            ...
            foreach(string category in new[]{"Skill","Companion"}) {
                var items=ui.Items(category);Assert.That(items.Count,Is.EqualTo(24));
                for(int grade=0;grade<6;grade++)Assert.That(items.Count(x=>x.rarity==grade),Is.EqualTo(4));
        ...
        public IEnumerator ExpansionVariantsEmitRequestedCountsSizesDirectionsAndFireParticles()
        {
            ...
            Assert.That(game.VariantProjectilesLaunched-before,Is.EqualTo(6+2+8+5));
            Assert.That(Vector3.Distance(cloud.transform.position,origin),Is.EqualTo(.84f).Within(.05f));
