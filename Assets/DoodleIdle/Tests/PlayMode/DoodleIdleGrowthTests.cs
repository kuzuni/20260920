...
namespace DoodleIdle.Tests
{
    public partial class DoodleIdlePlayModeTests
    {
        ...
        public IEnumerator GrowthRelicBulkYieldsAndConsumesUntilEmptyOrMaximumLevel()
        {
            ...
            relic.level = 1; relic.count = copies; relic.discovered = true;
            typeof(DoodleUi).GetField("collectionRandom", GrowthPrivate).SetValue(ui, new System.Random(421));
            var expectedRandom = new System.Random(421); int expectedWins = 0;
            for (int i = 0; i < copies; i++) if (expectedRandom.NextDouble() < .5) expectedWins++;
            long oldAttempts = ui.CareerProgress("relicAttempt"), oldWins = ui.CareerProgress("relicUpgrade");
            long wallet = ui.Gold;
            ...
            Assert.That(relic.count, Is.GreaterThan(0).And.LessThan(copies));
            ...
            int frames = 0;
            for (; frames < 200 && GrowthBulkRunning; frames++) yield return null;
            Assert.That(GrowthBulkRunning, Is.False);
            Assert.That(relic.count, Is.Zero);
            Assert.That(ui.Gold, Is.EqualTo(wallet));
            Assert.That(relic.level, Is.EqualTo(1 + expectedWins), "Batching must preserve the exact seeded per-copy random outcome.");
            Assert.That(ui.CareerProgress("relicAttempt"), Is.EqualTo(oldAttempts + copies));
            Assert.That(ui.CareerProgress("relicUpgrade"), Is.EqualTo(oldWins + expectedWins));
            Debug.Log("Bulk relic benchmark: 1,000,000 attempts, " + frames + " frames, " + stopwatch.ElapsedMilliseconds + " ms including UI/save.");
            ...
            int expectedUsed = 1;
            while (expectedRandom.NextDouble() >= .5) expectedUsed++;
            Assert.That(relic.count, Is.EqualTo(512 - expectedUsed));
