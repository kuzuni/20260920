...
namespace DoodleIdle
{
    public sealed partial class DoodleUi
    {
        ...
        void BuildBossHud()
        {
            ...
            bossHealthFill = BuildBossBar("Boss health bar", -20, new Color(.9f,.3f,.24f), out bossHealthLabel);
            bossTimerFill = BuildBossBar("Boss timer bar", -62, new Color(.2f,.52f,.95f), out bossTimerLabel);
            bossHud.gameObject.SetActive(false);
        }
        RectTransform BuildBossBar(string name, float y, Color color, out Text label)
        {
            var bar = UiKit.Box(bossHud, name, UiKit.Paper, 34);
            ...
            var inner = UiKit.Rect(bar, "Fill area"); UiKit.Stretch(inner,4,4,4,4);
